...
<div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 40px; margin: 3rem 0;">
  <div style="display: flex; flex-direction: column; align-items: center; max-width: 350px; flex: 1; min-width: 280px;">
    <img src="/images/projets/win-infra-core/vmware-setup/vmware-nat-no-dhcp.png" alt="Désactivation DHCP VMware" style="width: 100%; height: auto; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);" />
  ...
  <div style="display: flex; flex-direction: column; align-items: center; max-width: 350px; flex: 1; min-width: 280px;">
    <img src="/images/projets/win-infra-core/vmware-setup/reseau-nat.png" alt="Configuration Réseau NAT" style="width: 100%; height: auto; border-radius: 10px; border: 1px solid rgba(255,255,255,0.1);" />
...
> 🛡️ **Hardening Réseau** : En entreprise, on désactive le DHCP sur les segments serveurs. Cela empêche l'attribution automatique d'IP à des équipements non autorisés et garantit la stabilité des futurs services DNS. Dans ce lab, le service DHCP VMware est désactivé afin de simuler un segment serveur maîtrisé.
En environnement réel, le DHCP serait centralisé sur un serveur dédié ou une appliance réseau.
...
Une fois l'ISO montée, nous démarrons la VM. Lors de la création, nous nommons l'inventaire VMware **`SRV-AD-01`** pour une organisation propre dès le départ.
...
1. **Mot de passe Administrateur** : Le mot de passe Administrateur local respecte une politique de complexité élevée et sera remplacé ultérieurement par des comptes nominaux après la promotion du domaine.
2. **VMware Tools** : Installation des pilotes pour optimiser la fluidité et les performances de la machine.
...
<div style="display: flex; flex-wrap: wrap; justify-content: center; gap: 40px; margin: 3rem 0;">
  <div style="display: flex; flex-direction: column; align-items: center; max-width: 400px;">
    ...
    <p style="font-size: 0.65rem; text-align: center; opacity: 0.5; margin-top: 8px; font-style: italic;">Définition du MDP Administrateur</p>
  </div>
...
Le paramétrage DNS est l'étape la plus critique. Pour un premier DC, on pointe sur soi-même. Aucun DNS public n’est configuré sur le contrôleur de domaine afin d’éviter toute résolution externe non maîtrisée. On peut accéder à la configuration avec la commande ncpa.cpl.
  ...
  <div class="overflow-hidden rounded-lg border border-white/10">
    <table class="w-full text-sm text-left">
      <thead class="bg-white/5">
        <tr>
          <th class="p-3 border-b border-white/10">Paramètre</th>
   ...
   <div style="display: flex; flex-direction: column; align-items: center; max-width: 400px;">
    ...
    <p style="font-size: 0.65rem; text-align: center; opacity: 0.5; margin-top: 8px; font-style: italic;">Configurer ip statique du réseau</p>
  </div>
</div>
...
Via le **Gestionnaire de serveur**, j'installe les binaires nécessaires. Je ne lance pas encore la promotion pour m'assurer que les rôles sont correctement déployés.

<div class="flex flex-col items-center mb-8">
  <img src="/images/projets/win-infra-core/roles/ajouter-role.png" alt="Menu Gérer" class="max-w-[500px] w-full rounded-lg border border-white/10" />
  <p class="text-[0.65rem] italic opacity-50 mt-2">Accès à l'assistant via le Gestionnaire de serveur</p>
</div>

### 1. AD DS (Active Directory Domain Services)
Le socle de l'annuaire permettant la gestion centralisée des identités.

<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  <div class="flex flex-col items-center">
    <img src="/images/projets/win-infra-core/roles/ad-install.png" alt="Sélection AD DS" class="rounded-lg border border-white/10 w-full" />
    <p class="text-[0.65rem] italic opacity-50 mt-2">Sélection du rôle AD DS</p>
  </div>
</div>

### 2. DNS Server
Essentiel pour la résolution des noms du domaine et la localisation des services via les enregistrements SRV.
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 my-6">
  <div class="flex flex-col items-center">
    ...
    <p class="text-[0.65rem] italic opacity-50 mt-2">Installation du rôle DNS</p>
  </div>
  <div class="flex flex-col items-center justify-center bg-white/5 rounded-lg border border-dashed border-white/20 p-4">
    <p class="text-sm text-center opacity-70">
      💡 <strong>Note :</strong> Le DNS est souvent installé automatiquement lors de la promotion du premier DC si le rôle n'est pas déjà présent.
    </p>
  </div>
</div>
...
<section class="mt-12">
  <p class="text-base leading-relaxed opacity-90">
    Cette étape met en évidence l’importance de la préparation d’un contrôleur de domaine avant sa promotion :
    réseau maîtrisé, configuration DNS cohérente, nommage définitif et réduction de la surface d’attaque.
  </p>

  <p class="text-base leading-relaxed opacity-90 mt-4">
    La prochaine phase est dédiée au <em>hardening</em> préalable, visant à renforcer la sécurité du serveur
    et à garantir une base saine avant son élévation en tant que contrôleur de domaine.
  </p>
</section>
